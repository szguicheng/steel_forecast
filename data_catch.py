...
with Progress(
    ...
    # 在此处输入需要爬取的材料名称
    material_name = [
        'JN235(RE)', 'JN255', 'JN255(RE)', 'JN345', 'JN345(RE)', 'JY235(RE)'
    ]
    # 在此处输入需要爬取的城市名称
    city_list = ['青岛', '琼海', '万宁']
    # 此处可以更改所需要的腐蚀数据字段(确保网站上的字段包含此处即可,无需精确匹配),如有更改,请一并更改下方代码的列标题df.columns以及数据类型转换以及字段数量append(在中间标明)
    ...
    #                     for _ in range(5):# 请修改此处的数字为所需的字段数量
    ...
    #     pass
    # df.to_excel(
    #     '/Users/guicheng/Documents/大学/本科学业/毕业设计/腐蚀数据处理/第二次腐蚀数据获取/JNJY/corrosion_temp_data.xlsx',
    #     index=False)
    ...
    # weather_data = []
    # # 在此处可以修改所需爬取的天气数据字段(确保网站上的字段包含此处即可,无需精确匹配),如有更改,请一并更改下方代码的列标题df.columns以及数据类型转换以及字段数量append(在中间标明)
    # weather_strings = [
    #     '试验地点', '试验时间', '平均温度', '平均相对湿度', '总辐射', '降水量', '降水时数', '平均风速',
    #     '瞬时法SO2', '瞬时法HCl', '日照时数', '海盐粒子', '硫酸盐化速率','最高温度','最低温度'
    # ]
    ...
    #                     for _ in range(14):# 请修改此处的数字为所需的字段数量
    #                         weather_data[i].append('')
    #                     for index, string in enumerate(weather_strings[1:],
    #                                                    start=1):
    #                         for tr in tr_list:
    #                             if tr.find('div',
    #                                        string=re.compile(string)) != None:
    ...
    #     df['日照时数'] = df['日照时数'].astype(float)
    #     df['海盐粒子'] = df['海盐粒子'].astype(float)
    #     df['硫酸盐化速率'] = df['硫酸盐化速率'].astype(float)
    #     df['最高温度'] = df['最高温度'].astype(float)
    #     df['最低温度'] = df['最低温度'].astype(float)
    # except Exception as e:
    #     pass
    # df.to_excel(
    #     '/Users/guicheng/Documents/大学/本科学业/毕业设计/腐蚀数据处理/第二次腐蚀数据获取/JNJY/weather_temp_data.xlsx',
    #     index=False)
    # progress.update(main_task, advance=1)
    ...
    # corr_df.to_excel(
    #     '/Users/guicheng/Documents/大学/本科学业/毕业设计/腐蚀数据处理/第二次腐蚀数据获取/JNJY/第二次腐蚀数据temp.xlsx'
    # )
    ...
    weather_df = pd.read_excel(
        '/Users/guicheng/Documents/大学/本科学业/毕业设计/腐蚀数据处理/第二次腐蚀数据获取/JNJY/weather_temp_data_human.xlsx'
    )
    ...
    for index, row in corr_df.iterrows():
        ...
        for string in data_string:
            selected_data_nonzero = selected_data[
                (selected_data[string] != 0) | (selected_data[string] != '') |
                (selected_data[string].notnull())]
    ...
    corr_df.to_excel(
        '/Users/guicheng/Documents/大学/本科学业/毕业设计/腐蚀数据处理/第二次腐蚀数据获取/JNJY/第二次腐蚀数据.xlsx',
        index=False)
    progress.update(main_task, advance=1)
